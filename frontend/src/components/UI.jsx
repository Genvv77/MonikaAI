      ...
      <div className="fixed inset-0 z-10 pointer-events-none flex flex-col justify-between h-[100dvh]">
        ...
        <div className="hidden md:flex fixed left-7 top-1/2 -translate-y-1/2 z-50 pointer-events-auto animate-fade-in-left">
          <div className="bg-white/40 backdrop-blur-xl border border-white/40 shadow-2xl shadow-purple-500/10 rounded-full p-2 flex flex-col items-center gap-3">
            ...
            <button onClick={() => alert("Crypto Mode Locked")} className={`w-12 h-12 rounded-full shadow-xl transition-all duration-300 border border-white/50 flex items-center justify-center ${mode === "crypto" ? "bg-blue-500 text-white" : "bg-white/80 text-gray-300 hover:bg-white/90 cursor-not-allowed"}`}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path fillRule="evenodd" d="M9.315 7.584C12.195 3.883 16.695 1.5 21.75 1.5a.75.75 0 0 1 .75.75c0 5.056-2.383 9.555-6.084 12.436h.003c-.384.25-.615.69-.615 1.164v2.7c0 .28.163.535.418.665l2.07 1.035a.75.75 0 0 1 .133 1.254l-4.5 2.25a.75.75 0 0 1-1.072-.983l1.373-3.66a.75.75 0 0 0-.272-.857l-1.92-1.37a.75.75 0 0 0-.857.27l-3.66 1.374a.75.75 0 0 1-.983-1.072l2.25-4.5a.75.75 0 0 1 1.254.133l1.035 2.07c.13.255.385.418.665.418h2.7c.474 0 .914-.23 1.164-.615V7.584Zm-2.33 3.43a2.25 2.25 0 1 0-3.182-3.182 2.25 2.25 0 0 0 3.182 3.182Z" clipRule="evenodd" /></svg></button>
            <div className="w-12 h-[2px] bg-gray-500/20 my-1"></div>
    ...
    <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
        ...
        >
          ...
          {history.map((msg, index) => (
             <div 
               key={index} 
               className={`relative group p-4 rounded-2xl text-sm backdrop-blur-md shadow-sm border border-white/40 animate-fade-in-up pointer-events-auto ${
                 msg.role === "user" 
        ...
        <div className="pointer-events-auto max-w-screen-md w-full mx-auto absolute bottom-4 left-1/2 -translate-x-1/2 px-4">
            ...
            <div className="flex items-end gap-2 w-full">
                ...
                <button
                    disabled={loading}
                    onClick={() => sendMessage()}
                    className={`h-[60px] px-8 rounded-2xl shadow-lg font-bold text-white transition-all flex items-center justify-center shrink-0 ${
                    loading ? "bg-pink-400 cursor-wait" : "bg-gradient-to-r from-pink-500 to-rose-500 hover:shadow-pink-500/30 hover:scale-105 active:scale-95"
                    }`}
                >
                    {loading ? (
